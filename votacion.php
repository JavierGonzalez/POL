...
if (($_GET[1] == 'verificacion') AND ($_GET[2]) AND (isset($pol['user_ID']))) {
	...
	if (is_numeric($_GET[2])) {
		$result = sql("SELECT * FROM votacion WHERE estado = 'borrador' AND ID = '".$_GET[2]."' LIMIT 1");
		$edit = r($result);
	}
...
} elseif ($_GET[1] == 'borradores') { // VER BORRADORES
