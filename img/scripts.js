...
$(document).ready(function() {
    ...
    }).mouseout(function() { clearTimeout(pnick);
        pnick = "";
...
function print_votonum(num) {
    ...
    if (num >= 10) { return "<span class=\"vcc\">+" + num + "</span>"; } else if (num >= 0) { return "<span class=\"vc\">+" + num + "</span>"; } else if (num > -10) { return "<span class=\"vcn\">" + num + "</span>"; } else { return "<span class=\"vcnn\">" + num + "</span>"; }
}
...
function msgkeydown(evt, elem) {
    obj = elem;
    var keyCode;
    if ("which" in evt) { keyCode = evt.which; } else if ("keyCode" in evt) { keyCode = evt.keyCode; } else if ("keyCode" in window.event) { keyCode = window.event.keyCode; } else if ("which" in window.event) { keyCode = evt.which; }
    if (keyCode == 9) {
...
function merge_list() {
    ...
    for (elnick in al) {
        if (al[elnick] < times_exp) {
            ...
            if (cargo_ID.substr(0, 3) == "98_") { var kick_nick = "ip-" + cargo_ID.substr(3); } else { var kick_nick = elnick; }
...
function delays() {
    ...
    chat_delay_close = setTimeout("chat_close()", 1800000); // 30min
}
