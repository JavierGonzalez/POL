...
$enviado = false;
$result = sql_old("SELECT ID, nick, api_pass, email FROM users WHERE email = '".$_POST['email']."' AND reset_last < '".$date."' LIMIT 1");
while ($r = r($result)) { 
    $enviado = true;
...
if ($enviado == false) {
    $nick_existe = false;
    $result = sql_old("SELECT ID FROM users WHERE email = '".$_POST['email']."' LIMIT 1");
