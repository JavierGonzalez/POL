...
$centro = '40.42,-3.70'; // Madrid


// GEN
if (!isset($pol['user_ID'])) {
	echo '<p>'._('El mapa de ciudadanos solo está disponible para ciudadanos').'.</p>';
	...
	if ($geo) {
		echo '<p>'._('Los ciudadanos más cercanos a ti').'.</p>';
	} else {
		echo '<p>'._('No estás geolocalizado').' '.boton(_('Geolocalízate'), '/geolocalizacion/fijar', false, 'large red').'</p>';
	}
...
<form action="/accion/geolocalizacion/add" method="POST">
...
<td align="right">'.boton(_('Eliminar tu geolocalización'), '/accion/geolocalizacion/del', _('¿Estás seguro de querer borrar tu geolocalización de forma permanente?\n\nDebes saber que -para proteger la privacidad- la precisión guardada es de 1112 metros a la redonda, aleatoriamente.'), 'red small').'</td>
...
$(document).ready(function(){ 
	'.($_GET[1]=='filtro'?'print_eventos("'.$_GET[2].'", "'.$_GET[3].'");':'print_eventos("ciudadanos", "");').'
});
...
while ($r = mysqli_fetch_array($result)) { 
	echo '<option value="cargo|'.$r['cargo_ID'].'">'.$r['nombre'].'</option>';
}
...
while ($r = mysqli_fetch_array($result)) { 
	echo '<option value="grupos|'.$r['grupo_ID'].'">'.$r['nombre'].'</option>';
}
...
if ($_GET[1] == 'fijar') { $txt_nav[] = _('Geolocalízate'); } 
elseif ($_GET[1] == 'vecinos') { $txt_nav[] = _('Ciudadanos cercanos'); } 
else { $txt_nav[] = '&nbsp;'; }
