...
ORDER BY num_lista DESC, afiliados DESC, nombre DESC", $link);
	while($row = mysql_fetch_array($result)){
		...
		$txt .= '<tr><td align="right" valign="top"><b style="font-size:20px;">' . crear_link($row['siglas'], 'partido') . '</b></td><td>' . $row['nombre'] . '</td><td><b>' . $row['afiliados'] . '</b>/' . $num_lista . '</td><td>' . crear_link($row['nick_presidente']) . '</td><td>' . $nick_candidato . '</td><td align="right">' . duracion(time() - strtotime($row['fecha_creacion'])) . '</td><td>' . $elecciones . '</td></tr>' . "\n";
	}
	$txt .= '</table>';
