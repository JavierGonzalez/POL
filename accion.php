...
OR (($pol['estado'] == 'kickeado') AND (in_array($_GET[1], array('rechazar-ciudadania', 'votacion'))))
OR (($pol['estado'] == 'extranjero') AND (in_array($_GET[1], array('voto', 'mercado', 'foro', 'votacion', 'api'))))
) {


//###################################################################
switch ($_GET[1]) { //############## BIG ACTION SWITCH ############
//###################################################################
	...
	case 'experimento-1':
		...
		$result = sql_old("SELECT item_ID, emisor_ID FROM votos WHERE tipo = 'confianza'");
		while($r = r($result)) {
...
/*case 'api':
	$refer_url = 'api/';
	if (($_GET[2] == 'crear') AND (is_numeric($_POST['api_ID']))) {
		$result = sql_old("SELECT * FROM api WHERE api_ID = '".$_POST['api_ID']."' LIMIT 1");
		while($r = r($result)) {
			if (nucleo_acceso($r['acceso_borrador'])) {
				if (is_numeric($_POST['post_ID'])) {
					sql_old("UPDATE api_posts SET time = '".$date."', time_cron = '".$_POST['time_cron']."', message = '".$_POST['message']."', picture = '".$_POST['picture']."', link = '".$_POST['link']."', source = '".$_POST['source']."' WHERE post_ID = '".$_POST['post_ID']."' AND estado != 'publicado' LIMIT 1");
				} else {
					sql_old("INSERT INTO api_posts (pais, api_ID, estado, pendiente_user_ID, time, time_cron, message, picture, link, source) 
VALUES ('".PAIS."', '".$r['api_ID']."', 'pendiente', '".$pol['user_ID']."', '".$date."', '".trim($_POST['time_cron'])."', '".strip_tags(trim($_POST['message']))."', '".strip_tags(trim($_POST['picture']))."', '".strip_tags(trim($_POST['link']))."', '".strip_tags(trim($_POST['source']))."')");
	...
	} elseif (($_GET[2] == 'publicar') AND (is_numeric($_GET['ID']))) {
		api_facebook('publicar', $_GET['ID']);
	} elseif (($_GET[2] == 'borrar') AND (is_numeric($_GET['ID']))) {
		api_facebook('borrar', $_GET['ID']);
	} elseif (($_GET[2] == 'borrar_borrador') AND (is_numeric($_GET['ID']))) {
		$result = sql_old("SELECT api_ID, (SELECT acceso_escribir FROM api WHERE api_ID = api_posts.api_ID LIMIT 1) AS acceso_escribir FROM api_posts WHERE post_ID = '".$_GET['ID']."' LIMIT 1");
		while($r = r($result)) {
			if (nucleo_acceso($r['acceso_escribir'])) {
				sql_old("DELETE FROM api_posts WHERE post_ID = '".$_GET['ID']."' AND estado != 'publicado' LIMIT 1");
			}
			$refer_url = 'api/'.$r['api_ID'];
		}
	}
	...
	if ($_GET[2] != 'crear') {
		$result = sql_old("SELECT api_ID FROM api_posts WHERE post_ID = '".$_GET['ID']."' LIMIT 1");
...
case 'api':
	if (($pol['user_ID']) AND ($_GET[2] == 'gen_pass')) {
		mysql_query_old("UPDATE users SET api_pass = '".substr(md5(mt_rand(1000000000,9999999999)), 0, 12)."' WHERE ID = '".$pol['user_ID']."'", $link);
		$refer_url = 'perfil/editar/';
	}
	break;

case 'users_con':
	sql_old("DELETE FROM users_con WHERE user_ID = '".$pol['user_ID']."' ORDER BY time DESC LIMIT 1");
...
case 'socios':
	...
	if (($_GET[2] == 'inscribirse') AND (nucleo_acceso('ciudadanos')) AND ($es_socio == false) AND ($pol['config']['socios_estado'] == 'true') AND ($_POST['nombre']) AND ($_POST['NIF']) AND ($_POST['localidad']) AND ($_POST['cp']) AND ($_POST['contacto_email'])) {
		$last_socio_ID = 0;
		
		$last_socio_ID = sql_old("SELECT socio_ID FROM socios WHERE pais = '".PAIS."' ORDER BY socio_ID DESC LIMIT 1", true);
		
		sql_old("INSERT INTO socios (time, time_last, pais, socio_ID, user_ID, nombre, NIF, pais_politico, localidad, cp, direccion, contacto_email, contacto_telefono) 
VALUES ('".$date."', '".$date."', '".PAIS."', '".($last_socio_ID==0?10000:$last_socio_ID+1)."', '".$pol['user_ID']."', '".ucfirst(trim($_POST['nombre']))."', '".str_replace(' ', '', str_replace('-', '', strtoupper(trim($_POST['NIF']))))."', '".$_POST['pais_politico']."', '".ucfirst($_POST['localidad'])."', '".trim($_POST['cp'])."', '".ucfirst(trim($_POST['direccion']))."', '".strtolower(trim($_POST['contacto_email']))."', '".str_replace(' ', '', trim($_POST['contacto_telefono']))."')");
	...
	} elseif (($_GET[2] == 'cancelar') AND ($es_socio)) {
		sql_old("DELETE FROM socios WHERE pais = '".PAIS."' AND user_ID = '".$pol['user_ID']."' LIMIT 1");
		sql_old("UPDATE users SET socio = 'false' WHERE ID = '".$pol['user_ID']."'");
		if ($socio_estado == 'socio') {
			cargo_del($pol['config']['socios_ID'], $pol['user_ID']);
			sql_old("UPDATE users SET socio = 'false' WHERE ID = '".$pol['user_ID']."'");
		}

	} elseif (($_GET[2] == 'configurar') AND (nucleo_acceso($vp['acceso']['control_socios']))) {
		foreach (array('socios_estado', 'socios_ID', 'socios_descripcion', 'socios_responsable') AS $dato) {
			sql_old("UPDATE config SET valor = '".nl2br(strip_tags(trim($_POST[$dato])))."' WHERE pais = '".PAIS."' AND dato = '".$dato."'");
		}
		$refer_url = 'socios/configurar';

	} elseif (($_GET[2] == 'aprobar') AND (nucleo_acceso($vp['acceso']['control_socios'])) AND (is_numeric($_GET['ID']))) {
		$result = sql_old("SELECT ID, user_ID FROM socios WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND estado != 'socio' LIMIT 1");
		while($r = r($result)) {
			sql_old("UPDATE socios SET estado = 'socio', validador_ID = '".$pol['user_ID']."' WHERE ID = '".$_GET['ID']."'");
			sql_old("UPDATE users SET socio = 'true' WHERE ID = '".$r['user_ID']."'");
			cargo_add($pol['config']['socios_ID'], $r['user_ID']);
		}
		$refer_url = 'socios/inscritos';
	
	} elseif (($_GET[2] == 'rescindir') AND (nucleo_acceso($vp['acceso']['control_socios'])) AND (is_numeric($_GET['ID']))) {
		$refer_url = 'socios/asociados';
		$result = sql_old("SELECT ID, user_ID, estado FROM socios WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' LIMIT 1");
		while($r = r($result)) {
			sql_old("UPDATE users SET socio = 'false' WHERE ID = '".$r['user_ID']."'");
			if ($r['estado'] == 'socio') {
				sql_old("UPDATE socios SET estado = 'rescindido', validador_ID = '".$pol['user_ID']."' WHERE ID = '".$_GET['ID']."'");
				cargo_del($pol['config']['socios_ID'], $r['user_ID']);
			} else { 
				sql_old("DELETE FROM socios WHERE ID = '".$_GET['ID']."' LIMIT 1");
				$refer_url = 'socios/inscritos'; 
			}
		}
	}
...
case 'grupos':
	if (($_GET[2] == 'crear') AND (nucleo_acceso($vp['acceso']['control_grupos']))) {
		sql_old("INSERT INTO grupos (pais, nombre) VALUES ('".PAIS."', '".ucfirst($_POST['nombre'])."')");
	} elseif (($_GET[2] == 'eliminar') AND (nucleo_acceso($vp['acceso']['control_grupos'])) AND ($_GET['grupo_ID'])) {
		sql_old("DELETE FROM grupos WHERE grupo_ID = '".$_GET['grupo_ID']."' AND pais = '".PAIS."' LIMIT 1");
	} elseif ($_GET[2] == 'afiliarse') {
		$grupos_array = array();
		$result = sql_old("SELECT * FROM grupos WHERE pais = '".PAIS."'");
		while($r = r($result)) {
			...
			if ($_POST['grupo_'.$r['grupo_ID']] == 'true') {
				...
				if (!nucleo_acceso('grupos', $r['grupo_ID'])) {
					sql_old("UPDATE grupos SET num = num + 1 WHERE grupo_ID = '".$r['grupo_ID']."'");
				}
			} else {
				if (nucleo_acceso('grupos', $r['grupo_ID'])) {
					sql_old("UPDATE grupos SET num = num - 1 WHERE grupo_ID = '".$r['grupo_ID']."'");
		...
		sql_old("UPDATE users SET grupos = '".implode(' ', $grupos_array)."' WHERE ID = '".$pol['user_ID']."'");
	}
	$refer_url = 'grupos';
	break;
...
case 'perfil':
	if ($_GET[2] == 'datos') {		
		...
		sql_old("UPDATE users SET datos = '".implode('][', $datos_array)."' WHERE ID = '".$pol['user_ID']."'");
	} elseif ($_GET[2] == 'nombre') {
		sql_old("UPDATE users SET nombre = '".strip_tags($_POST['nombre'])."' WHERE ID = '".$pol['user_ID']."'");
	}
	$refer_url = 'perfil/editar';
	break;

case 'aceptar-condiciones':
	$result = sql_old("SELECT ID FROM users WHERE ID = '".$pol['user_ID']."' AND fecha_legal = '0000-00-00 00:00:00' LIMIT 1");
	while($r = r($result)) {
		sql_old("UPDATE users SET fecha_legal = '".$date."' WHERE ID = '".$pol['user_ID']."'");
...
case 'donacion':
	sql_old("UPDATE users SET donacion = ".($_POST['donacion']>=5?"'".$_POST['donacion']."'":"NULL")." WHERE ID = '".$pol['user_ID']."'");
	$refer_url = 'perfil/'.$pol['nick'];
	break;


case 'SC':
	if (($_GET[2] == 'nota') AND (nucleo_acceso('supervisores_censo')) AND ($_GET['ID'])) {
		sql_old("UPDATE users SET nota_SC = '".strip_tags($_POST['nota_SC'])."' WHERE ID = '".$_GET['ID']."'");
...
case 'bloqueos':
	if (nucleo_acceso('supervisores_censo')) {
		foreach (array('backlist_emails', 'backlist_IP', 'backlist_nicks') AS $tipo) {
			...
			if (count($data) > 0) { sort($data); sql_old("UPDATE config SET valor = '".implode("\n", $data)."' WHERE dato = '".$tipo."'"); }
		}
...
case 'exencion_impuestos':
	if ($pol['nivel'] >= 98) {
		$result = sql_old("SELECT ID, exenta_impuestos FROM cuentas WHERE pais = '".PAIS."' AND nivel = '0'");
		while($r = r($result)) {
			if (($_POST['exenta_impuestos'.$r['ID']] == '1') AND ($r['exenta_impuestos'] == '0')) {
				sql_old("UPDATE cuentas SET exenta_impuestos = 1 WHERE pais = '".PAIS."' AND ID = '".$r['ID']."'");
			}
			elseif  (!isset($_POST['exenta_impuestos'.$r['ID']]) AND ($r['exenta_impuestos'] == '1')) {
				sql_old("UPDATE cuentas SET exenta_impuestos = 0 WHERE pais = '".PAIS."' AND ID = '".$r['ID']."'");
			}
		}
...
case 'chat':
	...
	if (($_GET[2] == 'solicitar') AND ($_POST['nombre'])) {
		$nombre = $_POST['nombre'];
		$url = gen_url($nombre);

		sql_old("INSERT INTO chats (pais, url, titulo, user_ID, admin, fecha_creacion, fecha_last, dias_expira) 
VALUES ('".PAIS."', '".$url."', '".ucfirst($nombre)."', '".$pol['user_ID']."', '".$pol['nick']."', '".$date."', '".$date."', '".$pol['config']['chat_diasexpira']."')");
		if (ECONOMIA) {
			$result = sql_old("SELECT chat_ID FROM chats WHERE url = '".$url."' AND user_ID = '".$pol['user_ID']."' AND pais = '".$_POST['pais']."' LIMIT 1");
			while($r = r($result)) {
		...
		$result = sql_old("SELECT admin, user_ID, url FROM chats WHERE chat_ID = '".$_POST['chat_ID']."' AND estado = 'activo' LIMIT 1");
		while($r = r($result)) {
			if ((nucleo_acceso('privado', $r['admin'])) OR ($r['user_ID'] == $pol['user_ID']) OR (nucleo_acceso($vp['acceso']['control_gobierno']))) {
				sql_old("UPDATE chats SET admin = '".strtolower(strip_tags($_POST['admin']))."' WHERE chat_ID = '".$_POST['chat_ID']."'");
			}
...
WHERE chat_ID = '".$_POST['chat_ID']."' AND estado = 'activo' AND pais = '".PAIS."'");
			}
		}
		...
		sql_old("UPDATE chats SET estado = 'activo' WHERE chat_ID = '".$_GET['chat_ID']."' AND estado != 'activo' AND pais = '".PAIS."'");
		$refer_url = 'chat/list';
		...
		sql_old("UPDATE chats SET estado = 'bloqueado' WHERE chat_ID = '".$_GET['chat_ID']."' AND pais = '".PAIS."' AND user_ID = '".$pol['user_ID']."' OR 'true' = '".(nucleo_acceso($vp['acceso']['control_gobierno'])?'true':'false')."')");
		$refer_url = 'chat/list';
	}
	break;
...
case 'historia':
	...
	if (($_GET[2] == 'add') AND ($_POST['hecho'] != '')) {
		sql_old("INSERT INTO hechos (time, nick, texto, estado, time2, pais) VALUES ('".$_POST['year']."-".$_POST['mes']."-".$_POST['dia']."', '".$pol['nick']."', '".strip_tags($_POST['hecho'],'<b>,<a>')."', 'ok', '".$date."', '".$_POST['pais']."')");
	} elseif ($_GET[2] == 'del') {
		sql_old("UPDATE hechos SET estado = 'del' WHERE ID = '".$_GET['ID']."' AND (nick = '".$pol['nick']."' OR '".$pol['nivel']."' = '100' OR '".$sc[$pol['user_ID']]."' != '')");
	}
...
case 'geolocalizacion':
	if (($_GET[2] == 'add') AND (is_numeric($_POST['x'])) AND (is_numeric($_POST['y']))) {
		...
		$result = sql_old("SELECT ID FROM users WHERE ID = '".$pol['user_ID']."' AND x IS NULL LIMIT 1");
		while($r = r($result)) {
			evento_chat('<b>[#]</b> '.crear_link($pol['nick']).' se ha geolocalizado en el <a href="/geolocalizacion"><b>mapa</b> de ciudadanos</a>');
		}
		sql_old("UPDATE users SET x = '".$_POST['x']."', y = '".$_POST['y']."' WHERE ID = '".$pol['user_ID']."'");
		
	} elseif ($_GET[2] == 'del') {
		sql_old("UPDATE users SET x = NULL, y = NULL WHERE ID = '".$pol['user_ID']."'");
	}
...
case 'pass':
	if (($pol['user_ID'] == 1) AND ($_GET['nick'])) {
		...
		$result = sql_old("SELECT ID, nick, email FROM users WHERE nick = '".$_GET['nick']."' LIMIT 1");
		while($r = r($result)) {
			$email = $r['email'];
			$user_ID = $r['ID'];
			$nick = $r['nick'];
		...
		if ($_GET['nick'] == $nick) {
			...
			sql_old("UPDATE users SET pass = '".pass_key($new_pass, 'md5')."', pass2 = '".pass_key($new_pass)."', reset_last = fecha_registro WHERE ID = '".$user_ID."'");
...
LIMIT 1");
	...
	if (($user_ID) AND ($_POST['pais'] == PAIS) AND ($pols >=0)) { // RECHAZAR CIUDADANIA
		...
		sql_old("UPDATE users SET estado = 'turista', pais = 'ninguno', nivel = '1', cargo = '0', cargos = '', examenes = '', nota = '0.0', pols = '".$pols."', rechazo_last = '".$date."' WHERE ID = '".$pol['user_ID']."'");
...
case 'expulsar':
	$sc = get_supervisores_del_censo();
	if (($_GET[2] == 'desexpulsar') AND ($_GET['ID'])) {
		$result = sql_old("SELECT ID, user_ID, tiempo  FROM expulsiones WHERE ID = '".$_GET['ID']."' LIMIT 1");
		while ($r = r($result)) {
			sql_old("UPDATE users SET estado = 'ciudadano', fecha_last = '".$date."' WHERE ID = '".$r['user_ID']."'");
			sql_old("UPDATE expulsiones SET estado = 'cancelado' WHERE ID = '".$_GET['ID']."'");
		}
		...
		$result = sql_old("SELECT nick, ID FROM users WHERE nick IN ('".implode("','", explode(' ', $_POST['nick']))."') AND estado != 'expulsado' LIMIT 8");
		while ($r = r($result)) {
			sql_old("UPDATE users SET estado = 'expulsado' WHERE ID = '".$r['ID']."'");
			//sql_old("DELETE FROM votos WHERE emisor_ID = '".$r['ID']."'");

			// Cambia a "En Blanco" los votos. Es equivalente a anular el voto.
			$result2 = sql_old("SELECT ID, tipo_voto, respuestas FROM votacion WHERE estado = 'ok'");
			while ($r2 = r($result2)) { 
				$voto_en_blanco = '';
				...
				sql_old("UPDATE votacion_votos SET voto = '".$voto_en_blanco."', validez = 'true', mensaje = '' WHERE ref_ID = ".$r2['ID']." AND user_ID = ".$r['ID']."");
			}
...
case 'voto':
	...
	if ((in_array($tipo, $tipos_posibles)) AND (in_array($voto, $votos_posibles))) {
		...
		if ($tipo == 'confianza') {
			...
			$result = sql_old("SELECT COUNT(*) AS num FROM votos WHERE tipo = 'confianza' AND emisor_ID = '".$pol['user_ID']."' AND voto != '0'");
			...
			// existe usuario
			$result = sql_old("SELECT ID FROM users WHERE ID = '".$item_ID."'");
			...
			$pais = PAIS;
			$result = sql_old("SELECT ID FROM votacion_argumentos WHERE ID = '".$item_ID."' LIMIT 1");
			while ($r = r($result)) { $check = true; }
		} else {
			$pais = PAIS;
			$result = sql_old("SELECT ID FROM ".SQL."foros_".$tipo." WHERE ID = '".$item_ID."' AND user_ID != '".$pol['user_ID']."' LIMIT 1");
			while ($r = r($result)) { $check = true; }
		}
		...
		if ($check) {
			...
			$hay_voto = false;
			$result = sql_old("SELECT voto_ID FROM votos WHERE tipo = '".$tipo."' AND pais = '".$pais."' AND emisor_ID = '".$pol['user_ID']."' AND item_ID = '".$item_ID."' LIMIT 1");
			while ($r = r($result)) { $hay_voto = $r['voto_ID']; }

			if ($hay_voto != false) {
				sql_old("UPDATE votos SET voto = '".$voto."', time = '".$date."' WHERE voto_ID = '".$hay_voto."'");
			} else {
				sql_old("INSERT INTO votos (item_ID, pais, emisor_ID, voto, time, tipo) VALUES ('".$item_ID."', '".$pais."', '".$pol['user_ID']."', '".$voto."', '".$date."', '".$tipo."')");
			}

			// Contadores
			if (($tipo == 'hilos') OR ($tipo == 'msg')) {
				$result = sql_old("SELECT SUM(voto) AS num, COUNT(*) AS votos_num FROM votos WHERE tipo = '".$tipo."' AND pais = '".$pais."' AND item_ID = '".$item_ID."' AND (voto = 1 OR voto = -1)");
				while ($r = r($result)) { 
					$voto_result = $r['num'];
					sql_old("UPDATE ".SQL."foros_".$tipo." SET votos = '".$r['num']."', votos_num = '".$r['votos_num']."' WHERE ID = '".$item_ID."'");
				}
			} elseif ($tipo == 'argumentos') {
				$result = sql_old("SELECT SUM(voto) AS num, COUNT(*) AS votos_num FROM votos WHERE tipo = '".$tipo."' AND item_ID = '".$item_ID."' AND (voto = 1 OR voto = -1)");
				while ($r = r($result)) { 
					$voto_result = $r['num'];
					sql_old("UPDATE votacion_argumentos SET votos = '".$r['num']."', votos_num = '".$r['votos_num']."' WHERE ID = '".$item_ID."'");
				}
			}
		}
	}
...
case 'avatar':
	...
	if ($_GET[2] == 'upload') {
		...
		if (file_exists($img_root . $nom_file)) {
			...
			sql_old("UPDATE users SET avatar_localdir = '".e($_FILES['avatar']['name'])."', avatar = 'true' WHERE ID = '".$pol['user_ID']."'");
		}
	} elseif ($_GET[2] == 'borrar') {
		unlink($img_root.$pol['user_ID'].'.jpg');
		unlink($img_root.$pol['user_ID'].'_40.jpg');
		unlink($img_root.$pol['user_ID'].'_80.jpg');
		sql_old("UPDATE users SET avatar = 'false' WHERE ID = '".$pol['user_ID']."'");
		$refer_url = 'perfil/editar';
	} elseif (($_GET[2] == 'desc')) {
		$_POST['desc'] = gen_text($_POST['desc'], 'plain');
		sql_old("UPDATE users SET text = '".$_POST['desc']."' WHERE ID = '".$pol['user_ID']."'");
	}
	$refer_url = 'perfil/editar';
	break;
...
case 'examenes':
	if ($_GET[2] == 'eliminar-acceso-profesor'  AND (nucleo_acceso($vp['acceso']['examenes_decano'])) AND ($_GET[3]) AND ($_GET[4]) ){
		...
		sql_old("UPDATE examenes SET cargo_ID = '-" . $new_ID . "' WHERE pais = '".PAIS."' AND ID = '" . $new_ID . "'");
		evento_log('Examen nuevo '.$_POST['titulo']);
		$refer_url = 'examenes';


	} elseif (($_GET[2] == 'nueva-pregunta') AND ($_GET['ID'] != null) AND ((nucleo_acceso($vp['acceso']['examenes_decano'])) OR (nucleo_acceso($vp['acceso']['examenes_profesor']))) AND ($_POST['pregunta']) AND ($_POST['respuesta0'] != null) AND ($_POST['respuesta1'] != null) AND ($_POST['tiempo'])) {
		for ($i=0;$i<10;$i++) { 
		...
		sql_old("INSERT INTO examenes_preg (pais, examen_ID, user_ID, time, pregunta, respuestas, tiempo) VALUES ('".PAIS."', '" . $_GET['ID'] . "', '" . $pol['user_ID'] . "', '" . $date . "', '" . $pregunta . "', '" . $respuestas . "', " . $_POST['tiempo'] . ")");
		$refer_url = 'examenes/editar/' . $_GET['ID'];

	} elseif (($_GET[2] == 'eliminar-pregunta') AND ($_GET['ID'] != null) AND ((nucleo_acceso($vp['acceso']['examenes_decano'])) OR (nucleo_acceso($vp['acceso']['examenes_profesor'])))) {
		sql_old("DELETE FROM examenes_preg WHERE pais = '".PAIS."' AND ID = '" . $_GET['ID'] . "' LIMIT 1");
		$refer_url = 'examenes/editar/' . $_GET['re_ID'];

	} elseif (($_GET[2] == 'editar-examen') AND ($_GET['ID'] != null) AND (nucleo_acceso($vp['acceso']['examenes_decano'])) AND ($_POST['titulo']) AND ($_POST['descripcion']) AND ($_POST['nota'] >= 0) AND ($_POST['num_preguntas'] >= 0)) {
		$_POST['descripcion'] = gen_text($_POST['descripcion'], 'plain');
		sql_old("UPDATE examenes SET titulo = '".$_POST['titulo']."', descripcion = '".$_POST['descripcion'] . "', nota = '".$_POST['nota']."', num_preguntas = '".$_POST['num_preguntas']."' WHERE pais = '".PAIS."' AND ID = '" . $_GET['ID'] . "'");
		evento_log('Examen editado #'.$_GET['ID']);
		$refer_url = 'examenes/editar/'.$_GET['ID'];
		
	} elseif (($_GET[2] == 'examinar') AND ($_GET['ID'] != null) AND ($_POST['pregs']) AND (($_POST['tlgs'] + 10) > time())) {

		$result = sql_old("SELECT cargo_ID, titulo, ID, nota, num_preguntas,
...
FROM examenes WHERE pais = '".PAIS."' AND ID = '" . $_GET['ID'] . "' LIMIT 1");
		...
		if (($examen_ID) AND isset($_SESSION['examen'])) {
			if ($examen_ID == $_SESSION['examen']['ID']) {
				...
				sql_old("UPDATE cargos_users SET time = '".$date."', nota = '".$nota['nota']."'".$estado." WHERE pais = '".PAIS."' AND user_ID = '".$pol['user_ID']."' AND cargo_ID = '".$cargo_ID."'");

				if ($nota['nota'] >= $nota_aprobado) { // APROBADO
					
					$result2 = sql_old("SELECT cargo_ID FROM examenes WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND cargo_ID > 0 LIMIT 1");
					while($r2 = r($result2)){
						$result3 = sql_old("SELECT cargo_ID FROM cargos WHERE pais = '".PAIS."' AND cargo_ID = '".$r2['cargo_ID']."' AND autocargo = 'true' LIMIT 1");
...
FROM examenes WHERE pais = '".PAIS."' AND ID = '".$_POST['ID']."' LIMIT 1");
		...
		if ($_POST['pais'] == PAIS) {
			sql_old("DELETE FROM cargos_users WHERE cargo_ID = '".$_GET['ID']."' AND user_ID = '". $pol['user_ID']."' AND time < '".date('Y-m-d 20:00:00', time() - $pol['config']['examen_repe']*6)."' AND cargo_ID <= 0");
			actualizar('examenes');
			$refer_url = 'cargos';
		}
	} elseif (($_GET[2] == 'retirar_examen') AND (is_numeric($_GET['ID']))) {
		sql_old("DELETE FROM cargos_users WHERE cargo_ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."' LIMIT 1");
		actualizar('examenes');
...
case 'arquitecto':
	...
	if (($_GET[2] == 'comprar') AND ($_GET[3]) AND nucleo_acceso($vp['acceso']['gestion_mapa'])) {
		...
		if (isset($color)){
			sql_old("UPDATE mapa SET color = '".$color."' WHERE pais = '".PAIS."' AND estado = 'e'");
		}
		...
		$_POST['link'] = str_replace(HOST, "", $_POST['link']);
			sql_old("UPDATE mapa SET  text = '".$_POST['text']."', link = '".$_POST['link']."' WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND estado = 'e'");
		...
		if($r = r($result)){ 
			if ($r['ceder_user_ID']) {
				sql_old("UPDATE mapa SET user_ID = '".$r['ceder_user_ID']."', nick = '".$_POST['nick']."', estado='p', time = '".$date."' WHERE pais = '".PAIS."' AND ID = '".$r['ID']."'");
		...
		if($r = r($result)){ 
			for ($y=1;$y<=$r['size_y'];$y++) {
				for ($x=1;$x<=$r['size_x'];$x++) {
					if (($x==1) AND ($y==1)) {
						sql_old("UPDATE mapa SET size_x = 1, size_y = 1, superficie = 1, time = '".$date."', estado = 'e' WHERE pais = '".PAIS."' AND ID = '".$r['ID']."'");
						$puntero_x = $r['pos_x'];
...
case 'mapa':
	...
	if (($_GET[2] == 'compraventa') AND ($_GET['ID'])) {
		...
		$result = sql_old("SELECT ID, user_ID, pols FROM mapa WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND estado = 'v' AND '".$pol['pols']."' >= pols LIMIT 1");
		while($r = r($result)){ 
			if ($pol['user_ID'] != $r['user_ID']) {
				pols_transferir($r['pols'], $pol['user_ID'], $r['user_ID'], 'Compra-venta propiedad: '.$r['ID']);
				sql_old("UPDATE mapa SET estado = 'p', user_ID = '".$pol['user_ID']."', nick = '".$pol['nick']."' WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."'");
			}
		}
		$refer_url = 'mapa/';

	} elseif (($_GET[2] == 'cancelar-venta') AND ($_GET['ID'])) {

		sql_old("UPDATE mapa SET estado = 'p' WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."'");
		$refer_url = 'mapa/propiedades';

	} elseif (($_GET[2] == 'vender') AND ($_GET['ID']) AND ($_POST['pols'] > 0)) {

		sql_old("UPDATE mapa SET pols = '".$_POST['pols']."', estado = 'v' WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."'");
		$refer_url = 'mapa/propiedades';
	...
	} elseif (($_GET[2] == 'devolver') AND ($_GET['ID'])) {

		sql_old("DELETE FROM mapa WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND (user_ID = '".$pol['user_ID']."' OR (estado = 'e' AND 'true' = '".(nucleo_acceso($vp['acceso']['gestion_mapa'])?'true':'false')."')) LIMIT 1");
		$refer_url = 'mapa/propiedades';
		pols_transferir((($pol['config']['pols_solar'])*0.8), '-1',$pol['user_ID'],  'Devolucion propiedad: '.$_GET['ID']);
		if (strpos($_SERVER['HTTP_REFERER'], 'arquitecto') >= 0){
			$refer_url = 'mapa/arquitecto/propiedades';
		}
...
WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."' AND (estado = 'p' OR estado = 'e') LIMIT 1");
		while($r = r($result)){ 
			if ($r['ceder_user_ID']) {
				sql_old("UPDATE mapa SET user_ID = '".$r['ceder_user_ID']."', nick = '".$_POST['nick']."',  time = '".$date."' WHERE pais = '".PAIS."' AND ID = '".$r['ID']."'");
		...
		while($r = r($result)){ 
			...
			for ($y=1;$y<=$r['size_y'];$y++) {
				for ($x=1;$x<=$r['size_x'];$x++) {
					if (($x==1) AND ($y==1)) {
						sql_old("UPDATE mapa SET size_x = 1, size_y = 1, superficie = 1, time = '".$date."', estado = 'p' WHERE pais = '".PAIS."' AND ID = '".$r['ID']."'");
						$puntero_x = $r['pos_x'];
						$puntero['pos_x'] = $r['pos_x'];
						$puntero['pos_y'] = $r['pos_y'];
					} else {
						sql_old("INSERT INTO mapa (pais, pos_x, pos_y, size_x, size_y, user_ID, nick, link, text, time, pols, color, estado, superficie) VALUES ('".PAIS."', '".$puntero['pos_x']."', '".$puntero['pos_y']."', '1', '1', '".$pol['user_ID']."', '".$pol['nick']."', '".$r['link']."', '', '".$date."', '".$r['pols']."', '".$r['color']."', 'p', '1')");
					}
					$puntero['pos_x']++;
				}
				$puntero['pos_x'] = $puntero_x;
...
WHERE pais = '".PAIS."' AND (user_ID = '".$pol['user_ID']."' OR (estado = 'e' AND '1' = '".nucleo_acceso($vp['acceso']['gestion_mapa'])."')) AND (ID = '".$ID[0]."' OR ID = '".$ID[1]."') LIMIT 2");
		...
		if (($prop[$ID[0]]['size_x']) AND ($prop[$ID[1]]['size_x'])) {
			if ($_GET['f'] == 'x') {
				...
				sql_old("UPDATE mapa SET size_x = '".$size_x."', superficie = '".($size_x * $size_y)."' WHERE pais = '".PAIS."' AND ID = '".$ID[0]."'");
				//eliminar 1
				sql_old("DELETE FROM mapa WHERE pais = '".PAIS."' AND ID = '".$ID[1]."' LIMIT 1");
				...
				sql_old("UPDATE mapa SET size_y = '".$size_y."', superficie = '".($size_x * $size_y)."' WHERE pais = '".PAIS."' AND ID = '".$ID[0]."'");
				//eliminar 1
				sql_old("DELETE FROM mapa WHERE pais = '".PAIS."' AND ID = '".$ID[1]."' LIMIT 1");
...
/*		if ($hyperlink AND ($old_hyperlink != $hyperlink)){
		...
		if (preg_match("[^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$]", $_POST['color_1'])) {
			sql_old("UPDATE mapa SET color = '".$_POST['color_1']."', text = '".$_POST['text_1']."', link = '".$_POST['link_1']."' WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND (user_ID = '".$pol['user_ID']."' OR (estado = 'e' AND 'true' = '".(nucleo_acceso($vp['acceso']['gestion_mapa'])?'true':'false')."'))");
		}
		...
		for ($i = 2; $i<=$max_altura ; $i++){	
			...
			if (preg_match("[^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$]", $_POST['color_'.$i])) {
				...
				if ($r_existe = r($existe_mapa_altura)){
					...
					WHERE parcela_ID = '".$_GET['ID']."' AND altura = '".$i."'");
				}else{
					/*if ($hyperlink){
						webscreencapture($hyperlink, $img_root."parcela/".$_GET['ID']."_".$i.".png");
					}		*/		
...
($_POST['chat_diasexpira'] >= 10)
		...
		foreach ($_POST AS $dato => $valor) {
			if (substr($dato, 0, 8) != 'salario_') {
				...
				sql_old("UPDATE config SET valor = '".$valor."' WHERE pais = '".PAIS."' AND dato = '".$dato."'");
...
(entre($_POST['factor_propiedad'], -10, 10)) AND 
...
($_POST['arancel_salida'] <= 100) AND ($_POST['arancel_salida'] >= 0)
	...
	foreach ($_POST AS $dato => $valor) {
		...
		if ((substr($dato, 0, 8) != 'salario_') AND ($dato != 'palabra_gob1')  AND (substr($dato, 0, 7) != 'barrio_')) {
			...
			sql_old("UPDATE config SET valor = '".strip_tags($valor)."' WHERE pais = '".PAIS."' AND dato = '".$dato."'");
		...
		if ((substr($dato, 0, 7) == 'barrio_')) {
			...
			if ($old_value != $valor){
				error_log("UPDATE mapa_barrios SET ".$campo ." = '".strip_tags($valor)."' WHERE ID = '".$id_barrio."'");

				sql_old("UPDATE mapa_barrios SET ".$campo ." = '".strip_tags($valor)."' WHERE ID = '".$id_barrio."'");
	...
	if (ECONOMIA) {
			...
			while($r = r($result)){
				...
				if (($salario >= 0) AND ($salario <= 1000)) {
					...
					sql_old("UPDATE cargos SET salario = '".$salario."' WHERE pais = '".PAIS."' AND cargo_ID = '".$r['cargo_ID']."'");
				}
			}
		}
	...
	} elseif (($_GET[2] == 'categorias') AND (nucleo_acceso($vp['acceso']['control_gobierno']))) {
		
		if ($_GET[3] == 'editar') {
			$result = sql_old("SELECT ID FROM cat WHERE pais = '".PAIS."'");
			while ($r = r($result)) { 
				sql_old("UPDATE cat SET url = '". gen_url($_POST[$r['ID'].'_nombre'])."', nombre = '".$_POST[$r['ID'].'_nombre']."', nivel = '".$_POST[$r['ID'].'_nivel']."', orden = '".$_POST[$r['ID'].'_orden']."', publicar='".$_POST[$r['ID'].'_publicable']."' WHERE ID = '".$r['ID']."'");
			}
		} elseif ($_GET[3] == 'crear') {
			...
			sql_old("DELETE FROM cat WHERE ID = '".$_GET['ID']."' LIMIT 1");
		}
		
		evento_log('Gobierno configuración: categorías');
		$refer_url = 'control/gobierno/categorias';

	} elseif (($_GET[2] == 'privilegios') AND (nucleo_acceso($vp['acceso']['control_gobierno']))) {
		$_POST['control_socios'] = 'cargo';
		$result = sql_old("SELECT valor, dato FROM config WHERE pais = '".PAIS."' AND dato = 'acceso'");
		while ($r = r($result)) { $pol['config'][$r['dato']] = $r['valor']; }
		$accesos = array();
		...
		sql_old("UPDATE config SET valor = '".implode('|', $accesos)."' WHERE pais = '".PAIS."' AND dato = 'acceso'");
		evento_log('Gobierno configuración: privilegios');
		$refer_url = 'control/gobierno/privilegios';

	} elseif (($_GET[2] == 'subforo') AND (nucleo_acceso($vp['acceso']['control_gobierno']))) {
		$subforos = explode('.', $_POST['subforos']);
		foreach ($subforos AS $subforo_ID) {
			sql_old("UPDATE ".SQL."foros SET descripcion = '".$_POST[$subforo_ID.'_descripcion']."', time = '".$_POST[$subforo_ID.'_time']."', acceso_leer = '".$_POST[$subforo_ID.'_acceso_leer']."', acceso_escribir = '".$_POST[$subforo_ID.'_acceso_escribir']."', acceso_escribir_msg = '".$_POST[$subforo_ID.'_acceso_escribir_msg']."', acceso_cfg_leer = '".$_POST[$subforo_ID.'_acceso_cfg_leer']."', acceso_cfg_escribir = '".$_POST[$subforo_ID.'_acceso_cfg_escribir']."', acceso_cfg_escribir_msg = '".$_POST[$subforo_ID.'_acceso_cfg_escribir_msg']."', limite = '".$_POST[$subforo_ID.'_limite']."' WHERE ID = '".$subforo_ID."'");
		}
		evento_log('Gobierno configuración: foro');
		$refer_url = 'control/gobierno/foro';

	} elseif (($_GET[2] == 'crearsubforo') AND (nucleo_acceso($vp['acceso']['control_gobierno']))) {
		sql_old("INSERT INTO ".SQL."foros (url, title, descripcion, acceso, time, estado, acceso_msg) VALUES ('".gen_url($_POST['nombre'])."', '".$_POST['nombre']."', '', '1', '10', 'ok', '0')");
		evento_log('Gobierno configuración: foro '.$_POST['nombre']);
		$refer_url = 'control/gobierno/foro';

	} elseif (($_GET[2] == 'eliminarsubforo') AND (nucleo_acceso($vp['acceso']['control_gobierno'])) AND ($_GET['ID'])) {
		sql_old("DELETE FROM ".SQL."foros WHERE ID = '".$_GET['ID']."' LIMIT 1");
		evento_log('Gobierno configuración: foro eliminado #'.$_GET['ID']);
		$refer_url = 'control/gobierno/foro';
	
	} elseif (($_GET[2] == 'notificaciones') AND (nucleo_acceso($vp['acceso']['control_gobierno']))) {
		if (($_GET[3] == 'add') AND ($_POST['texto']) AND ($_POST['url'])) {
			...
			$result = sql_old("SELECT ID FROM users WHERE pais = '".PAIS."' AND ".sql_acceso($_POST['acceso'], $_POST['acceso_cfg'])." ORDER BY voto_confianza DESC LIMIT 100000");
		...
		} elseif (($_GET[3] == 'borrar') AND (is_numeric($_GET['noti_ID']))) {
			$result = sql_old("SELECT texto FROM notificaciones WHERE noti_ID = '".$_GET['noti_ID']."' LIMIT 1");
			while($r = r($result)){
				sql_old("DELETE FROM notificaciones WHERE texto = '".$r['texto']."' AND emisor = '".PAIS."'");
			}
...
case 'empresa':
	if (($_GET[2] == 'crear') AND ($pol['pols'] >= $pol['config']['pols_empresa']) AND (ctype_digit($_POST['cat'])) AND ($_POST['nombre'])) {
		$nombre = $_POST['nombre'];
		$url = gen_url($nombre);
		$result = sql_old("SELECT ID, url FROM cat WHERE pais = '".PAIS."' AND ID = '".$_POST['cat']."' LIMIT 1");
		while($r = r($result)){ $cat_url = $r['url']; $cat_ID = $r['ID']; }

		sql_old("INSERT INTO empresas (pais, url, nombre, user_ID, descripcion, web, cat_ID, time) VALUES ('".PAIS."', '".$url."', '".$nombre."', '".$pol['user_ID']."', 'Editar...', '', '".$cat_ID."', '".$date."')");

		sql_old("SELECT ID FROM ".SQL."vp_empresas WHERE nombre='$nombre'");
		$nick = $_SESSION['pol']['nick'];
		$acciones = 100;
		$acciones=sql_old("INSERT INTO ".SQL."acciones (nick, nombre_empresa, acciones, pais, ID_empresa) 
		VALUES ('".$nick."', '".$nombre."', '".$acciones."', '".$PAIS."')");

		sql_old("UPDATE cat SET num = num + 1 WHERE pais = '".PAIS."' AND ID = '".$cat_ID."'");

		pols_transferir($pol['config']['pols_empresa'], $pol['user_ID'], '-1', 'Creacion nueva empresa: '.$nombre);
		evento_log('Empresa creada ('.$nombre.')');
		$return = $cat_url.'/'.$url;

	} elseif (($_GET[2] == 'acciones') AND ($_GET['ID']) AND ($_POST['nick'] AND ($_POST['cantidad']))) {
		$id = $_GET['ID'];
		$result = sql_old("SELECT nombre, ID, user_ID FROM vp_empresas WHERE ID='$id', $link");
		if ($r=r($result)) {
			$id = $r['ID'];
			$nick = $_POST['nick'];
			$cantidad = $_POST['cantidad'];
			$id_user = $r['user_ID'];
			$acciones = sql_old("INSERT INTO acciones (ID_empresa, num_acciones, nick, pais) 
			VALUES ('".$id."', '".$cantidad."', '".$nick."', '".$PAIS."')");
			$usuario = sql_old("SELECT nick FROM vp_users WHERE ID = '$id_user', $link");
			if ($r=r($usuario)) {
				$nick = $r['nick'];
				$cantidadacciones = sql_old("SELECT acciones, nick, nombre_empresa FROM acciones WHERE nick = '$nick' and ID_empresa = '$id', $link");
				if ($r=r($cantidadacciones)) {
					$susacciones = $r['acciones'];
					$totalacciones = $susacciones - $cantidad;
					$accionesresultantes = sql_old("update acciones set acciones='$totalacciones' where nick='$nick' and ID_empresa='$id'");
				}
			}
		}
	} elseif (($_GET[2] == 'ceder') AND ($_GET['ID']) AND ($_POST['nick'])) {
...
WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."' LIMIT 1");
		while($r = r($result)){ 
			if ($r['ceder_user_ID']) {
				sql_old("UPDATE empresas SET user_ID = '".$r['ceder_user_ID']."' WHERE pais = '".PAIS."' AND ID = '".$r['ID']."'");
		...
		sql_old("UPDATE empresas SET descripcion = '".$txt."' WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."'");
		$return =  $_POST['return'];
		...
		sql_old("UPDATE empresas SET precio_suscripcion = '".$_POST['precio_suscripcion']."', periodicidad_suscripcion = '".$_POST['periodicidad_suscripcion']."' WHERE pais = '".PAIS."' AND ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."'");
		...
		VALUES('".$_GET['ID']."', '".$pol['user_ID']."', '".date('Y-m-d H:i:s')."', '".$precio_suscripcion."', '".$periodicidad_suscripcion."')");
		...
		while($r = r($result)){ 
			if ($r['user_ID'] == $pol['user_ID']){
				...
				if($usuario = r($obtener_usuario)){ 
					sql_old("INSERT INTO empresas_suscriptores
					(ID_empresa, ID_usuario, fecha_alta, precio_suscripcion, periodicidad_suscripcion)
					VALUES('".$_GET['ID']."', '".$usuario['ID']."', '".date('Y-m-d H:i:s')."', 0, 'U')");
					evento_log('Nuevo suscriptor #'.$_GET['ID']);
				}
			}
		}
		...
		WHERE ID = '".$_POST['cuenta']."' AND user_ID = '".$pol['user_ID']."'");
		...
		VALUES('false', '".$pol['user_ID']."', '".$ID_empresa."', '".$html_doc."', '".$titulo."', '".$adelanto_doc."', '".$precio_suscripcion."','".$precio."', 0, 0, '".date('Y-m-d H:i:s')."');
		");
...
case 'mercado':
	if (($_GET[2] == 'puja') AND ($pol['estado'] != 'extranjero') AND ($_GET['ID']) AND ($_POST['puja'] > 0) AND (is_numeric($_POST['puja'])) AND (date('H:i') != '20:00')) {
		$ID = $_GET['ID'];
		$pols = $_POST['puja'];
	...
	} elseif (($_GET[2] == 'editarfrase') AND (($pol['config']['pols_fraseedit'] == $pol['user_ID']) OR (nucleo_acceso($vp['acceso']['control_gobierno'])))) {
		$_POST['url'] = str_replace(array('http://', 'https://', ':', ',', ' '), '', $_POST['url']);
		$url = '<a href="http://'.strip_tags($_POST['url']).'">'.ucfirst(strip_tags($_POST['frase'])).'</a>';
		sql_old("UPDATE config SET valor = '".$url."' WHERE pais = '".PAIS."' AND dato = 'pols_frase'");
		evento_log('Frase editada');
		$refer_url = 'subasta/editar';

	} elseif (($_GET[2] == 'cederfrase') AND ($pol['config']['pols_fraseedit'] == $pol['user_ID']) AND ($pol['nick'] != $_POST['nick'])) {
		$result = sql_old("SELECT ID, nick, pais FROM users WHERE nick = '".$_POST['nick']."' AND estado = 'ciudadano' LIMIT 1");
		while($r = r($result)){ 
			sql_old("UPDATE config SET valor = '".$r['ID']."' WHERE pais = '".PAIS."' AND dato = 'pols_fraseedit'");	
		...
		sql_old("UPDATE config SET valor = '".$dato."' WHERE pais = '".PAIS."' AND dato = 'palabras'");
		evento_log('Enlace editado #'.$_GET['ID']);
		$refer_url = 'subasta/editar';

	} elseif (($_GET[2] == 'cederpalabra') AND ($_GET['ID'] >= 0) AND ($pol['nick'] != $_POST['nick'])) {
		$result = sql_old("SELECT ID, nick, pais FROM users WHERE nick = '".$_POST['nick']."'AND estado = 'ciudadano' LIMIT 1");
		while($r = r($result)){ 
			$dato = '';
			foreach(explode(";", $pol['config']['palabras']) as $num => $t) {
...
case 'pols':
	...
	if ($_GET[2] == 'apoderado'){
		if ($_GET[3] == 'anadir'){
			...
			if($r = r($result)){ 
				...
				if($resultado_comprobacion = r($comprobacion_cuenta)){
					sql_old("INSERT INTO cuentas_apoderados (cuenta_ID, user_ID, `time`) values ('".$resultado_comprobacion['ID']."','".$r['ID']."','".date('Y-m-d H:i:s')."')");
				}else{
		...
		if (($origen) AND ($destino)) { //todo OK
			...
			//quitar
			if ($origen == 'ciudadano') {
				sql_old("UPDATE users SET pols = pols - ".$pols." WHERE ID = '".$pol['user_ID']."'");
				...
				sql_old("UPDATE cuentas SET pols = pols - ".$pols." WHERE pais = '".PAIS."' AND ID = '".$_POST['origen']."'");
			...
			if ($destino == 'ciudadano') {
				...
				sql_old("UPDATE users SET pols = pols + ".$pols." WHERE ID = '".$destino_user_ID."'");
				$receptor_ID = $destino_user_ID;
			} elseif ($destino == 'cuenta') {
				sql_old("UPDATE cuentas SET pols = pols + ".$pols." WHERE pais = '".PAIS."' AND ID = '".$destino_cuenta_ID."'");
			...
			if (($pols > 0) AND ($emisor_ID != $receptor_ID)) {		
				...
				if ($transf_int) {
					sql_old("INSERT INTO transacciones (pais, pols, emisor_ID, receptor_ID, concepto, time) VALUES ('".$pais_destino."', '".$pols."', '".$emisor_ID."', '".$receptor_ID."', '".$concepto."', '".$date."')");
				}
				$refer_url = 'pols#ok';
			}
		}
		...
		sql_old("UPDATE cuentas set user_ID = (SELECT ID FROM users WHERE nick = '".strtolower($ciudadano)."') WHERE pais = '".PAIS."' AND ID = '".$cuenta."' AND user_ID = '".$pol['user_ID']."'");
...
case 'votacion':
	...
	if (($_GET[2] == 'crear') AND ($_POST['tipo'] != 'elecciones') AND (in_array($_POST['tipo'], $votaciones_tipo)) AND (nucleo_acceso($vp['acceso']['votacion_borrador']))) {
		...
		switch ($_POST['tipo']) {
			...
			case 'cargo':
				$result = sql_old("SELECT nombre FROM cargos WHERE cargo_ID = '".$_POST['cargo']."' AND pais = '".PAIS."'  LIMIT 1");
				while($r = r($result)){ $cargo_nombre = $r['nombre']; }

				$result = sql_old("SELECT ID, nick FROM users WHERE nick = '".$_POST['nick']."' AND pais = '".PAIS."' LIMIT 1");
...
WHERE estado = 'borrador' AND ID = '".$_POST['ref_ID']."' AND pais = '".PAIS."'");
	...
	} elseif (($_GET[2] == 'iniciar') AND (is_numeric($_GET['ref_ID']))) {
		
		$result = sql_old("SELECT * FROM votacion WHERE ID = '".$_GET['ref_ID']."' AND estado = 'borrador' LIMIT 1");
		while($r = r($result)){
			if (nucleo_acceso($vp['acceso'][$r['tipo']])) {
				...
				sql_old("UPDATE votacion SET estado = 'ok', user_ID = '".$pol['user_ID']."', time = '".$date."', time_expire = '".$r['time_expire']."', num_censo = '".$censo_num."' WHERE ID = '".$r['ID']."'");
		...
		evento_log('Votación: iniciada <a href="/votacion/'.$_GET['ref_ID'].'">#'.$_GET['ref_ID'].'</a>');
			...
			error_log("select id from users where id= '".$pol['user_ID']."' AND datediff('".date('Y-m-d H:i:s')."',GREATEST(fecha_registro , rechazo_last)) > 2");
			$result = sql_old("
                SELECT id
                FROM users
                WHERE id = '".$pol['user_ID']."'
                    AND (strftime('%s', CURRENT_TIMESTAMP) - strftime('%s', MAX(fecha_registro, rechazo_last))) > 172800
                ");
            ...
            if($r = r($result)){ 
				...
				if (($estado == 'ok') AND (in_array($tipo, $votaciones_tipo)) AND (nucleo_acceso($acceso_votar,$acceso_cfg_votar)) AND (nucleo_acceso($acceso_ver, $acceso_cfg_ver))) {
					...
					if ($ha_votado) {	// MODIFICAR VOTO
						sql_old("UPDATE votacion_votos SET voto = '".$_POST['voto']."', validez = '".$_POST['validez']."', mensaje = '".$_POST['mensaje']."' WHERE ref_ID = '".$_POST['ref_ID']."' AND user_ID = '".$pol['user_ID']."'");
					
					} else {			// INSERTAR VOTO
						
						sql_old("UPDATE votacion SET num = num + 1 WHERE ID = '".$_POST['ref_ID']."'");
		...
		while($r = r($result)) {
			if (($r['user_ID'] == $pol['user_ID']) OR (nucleo_acceso($vp['acceso'][$r['tipo']]))) {
				sql_old("DELETE FROM votacion WHERE ID = '".$r['ID']."' LIMIT 1");
				sql_old("DELETE FROM votacion_votos WHERE ref_ID = '".$r['ID']."'");
			}
		}

	} elseif (($_GET[2] == 'finalizar') AND (is_numeric($_GET['ID']))) { 
		$result = sql_old("SELECT ID, user_ID, estado, tipo FROM votacion WHERE estado != 'end' AND ID = '".$_GET['ID']."' AND pais = '".PAIS."' LIMIT 1");
		while($r = r($result)) {
			if (($r['user_ID'] == $pol['user_ID']) OR (($r['estado'] == 'borrador') AND (nucleo_acceso($vp['acceso'][$r['tipo']])))) {
				sql_old("UPDATE votacion SET estado = 'borrador', num = '0' WHERE ID = '".$r['ID']."'");
				sql_old("DELETE FROM votacion_votos WHERE ref_ID = '".$r['ID']."'");
			}
		}
		evento_log('Votación: cancelada <a href="/votacion/'.$_GET['ID'].'">#'.$_GET['ID'].'</a>');

	} elseif (($_GET[2] == 'enviar_comprobante') AND ($_GET['comprobante'])) {
	...
	} elseif (($_GET[2] == 'argumento') AND (is_numeric($_POST['ref_ID'])) AND (strlen($_POST['texto']) > 1)) {
		$result = sql_old("SELECT * FROM votacion WHERE estado != 'end' AND ID = '".$_POST['ref_ID']."' LIMIT 1");
		while($r = r($result)) {
			$_POST['sentido'] = substr(strip_tags($_POST['sentido']), 0, 40);
			if (nucleo_acceso($r['acceso_votar'], $r['acceso_cfg_votar'])) {
				sql_old("INSERT INTO votacion_argumentos (ref_ID, user_ID, time, sentido, texto) VALUES ('".$r['ID']."', '".$pol['user_ID']."', '".tiempo()."', '".$_POST['sentido']."', '".ucfirst(trim(substr(strip_tags($_POST['texto']), 0, 180)))."')");
				if (in_array($r['acceso_ver'], array('anonimos', 'ciudadanos_global', 'ciudadanos'))) {
	...
	} elseif (($_GET[2] == 'argumento-eliminar') AND (is_numeric($_GET['ID']))) {
		sql_old("DELETE FROM votacion_argumentos WHERE ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."' LIMIT 1");
...
case 'foro':
	// añadir, editar
	if ((($_GET[2] == 'reply') OR ($_GET[2] == 'hilo')) AND (strlen($_POST['text']) > 1) AND ($_POST['subforo'])) {
		if ($_POST['subforo'] == -1) { $acceso['escribir_msg'] = true; } else { 
			$acceso = false;
			$result = sql_old("SELECT acceso_leer, acceso_escribir, acceso_cfg_escribir, acceso_escribir_msg, acceso_cfg_escribir_msg FROM ".SQL."foros WHERE ID = '".$_POST['subforo']."' LIMIT 1");
			while($r = r($result)) { 
		...
		if (($_GET[2] == 'hilo') AND ($_POST['title']) AND ($acceso['escribir'])) {
            ...
            $result = sql_old("SELECT COUNT(*) AS num FROM ".SQL."foros_hilos WHERE url = '".$url."'");
			while($r = r($result)) { 
                if ($r['num'] > 0)
                    $url = $url.'-'.time();
            }
            ...
            */
		...
		} elseif (($_GET[2] == 'reply') AND ($acceso['escribir_msg'])) {
			if ($_POST['hilo'] != -1) {
				sql_old("UPDATE ".SQL."foros_hilos SET time_last = '".$date."' WHERE ID = '".$_POST['hilo']."'");
				$result = sql_old("SELECT title, num FROM ".SQL."foros_hilos WHERE ID = '".$_POST['hilo']."'");
			...
			sql_old("INSERT INTO ".SQL."foros_msg (hilo_ID, user_ID, time, text, cargo) VALUES ('".$_POST['hilo']."', '".$pol['user_ID']."', '".$date."', '".$text."', '".$_POST['encalidad']."')");
		}
		if ($_POST['hilo']) {
			$msg_num = 0;
			$result = sql_old("SELECT COUNT(*) AS num FROM ".SQL."foros_msg WHERE hilo_ID = '".$_POST['hilo']."' AND estado = 'ok'");
			while($r = r($result)) { $msg_num = $r['num']; }
			sql_old("UPDATE ".SQL."foros_hilos SET num = '".$msg_num."' WHERE ID = '".$_POST['hilo']."'");
		}
		...
		$result = sql_old("SELECT ID FROM users WHERE pais = '".PAIS."' AND estado = 'ciudadano' AND nick IN ('".implode("','", $nombrados)."') LIMIT 25");
	...
	} elseif (($_GET[2] == 'borrar') AND ($_GET['ID']) AND ($_GET[3]) AND (nucleo_acceso($vp['acceso']['foro_borrar']))) {
		$result = sql_old("SELECT user_ID FROM ".SQL."foros_".($_GET[3]=='hilo'?'hilo':'msg')." WHERE ID = '".$_GET['ID']."' LIMIT 1");
		while($r = r($result)){ $el_user_ID = $r['user_ID']; }
		if ($_GET[3] == 'hilo') {
			sql_old("UPDATE ".SQL."foros_hilos SET estado = 'borrado', time_last = '".$date."' WHERE ID = '".$_GET['ID']."' AND estado = 'ok'");
		} elseif ($_GET[3] == 'mensaje') {
			sql_old("UPDATE ".SQL."foros_msg SET estado = 'borrado', time2 = '".$date."' WHERE ID = '".$_GET['ID']."' AND estado = 'ok'");
		}
		evento_log('Foro <em>'.strtolower($_GET[3]).'</em> enviado a la papelera por moderación #'.$_GET['ID']);
		$refer_url = 'foro/papelera';

	} elseif (($_GET[2] == 'restaurar') AND ($_GET['ID']) AND ($_GET[3]) AND (nucleo_acceso($vp['acceso']['foro_borrar']))) {
		if ($_GET[3] == 'hilo') {
			sql_old("UPDATE ".SQL."foros_hilos SET estado = 'ok' WHERE ID = '".$_GET['ID']."' AND estado = 'borrado'");
		} elseif ($_GET[3] == 'mensaje') {
			sql_old("UPDATE ".SQL."foros_msg SET estado = 'ok', time2 = '0000-00-00 00:00:00' WHERE ID = '".$_GET['ID']."' AND estado = 'borrado'");
		}
		evento_log('Foro '.$_GET[3].' restaurado desde la papelera por moderación #'.$_GET['ID']);
		$refer_url = 'foro/papelera';


	} elseif (($_GET[2] == 'eliminarhilo') AND ($_GET['ID'])) {
		$result = sql_old("SELECT ID FROM ".SQL."foros_hilos WHERE ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."' LIMIT 1");
		while($r = r($result)){
			sql_old("DELETE FROM ".SQL."foros_hilos WHERE ID = '".$r['ID']."' LIMIT 1");
			sql_old("DELETE FROM ".SQL."foros_msg WHERE hilo_ID = '".$r['ID']."'");
		}
		evento_log('Foro hilo eliminado #'.$_GET['ID']);
		$refer_url = 'foro';

	} elseif (($_GET[2] == 'eliminarreply') AND ($_GET['hilo_ID']) AND ($_GET['ID'])) {
		...
		if ($es_ok) {
			sql_old("DELETE FROM ".SQL."foros_msg WHERE ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."' LIMIT 1");
			sql_old("UPDATE ".SQL."foros_hilos SET num = num-1 WHERE ID = '".$_GET['hilo_ID']."'");
			evento_log('Foro mensaje eliminado #'.$_GET['ID']);
		}
		if ($_GET['hilo_ID'] == '-1') { $refer_url = 'notas'; } else { $refer_url = 'foro'; }

	} elseif (($_GET[2] == 'editar') AND ($_POST['text']) AND ($_POST['subforo'])) {
		$text = gen_text($_POST['text'], 'plain');

		if ($_POST['hilo']) { //msg
			$result = sql_old("SELECT f.url foro, h.url hilo, m.ID mensaje FROM ".SQL."foros f, ".SQL."foros_hilos h, ".SQL."foros_msg m where f.ID = h.sub_ID and m.hilo_ID = h.ID and m.ID='".$_POST['hilo']."'");
			$r =r($result);

			sql_old("UPDATE ".SQL."foros_msg SET text = '".$text."' WHERE ID = '".$_POST['hilo']."' AND estado = 'ok' AND user_ID = '".$pol['user_ID']."' AND time > '".date('Y-m-d H:i:s', time() - 3600)."'");
			...
			if (strlen($_POST['title']) >= 4) {
				...
				$title = strip_tags($_POST['title']);
				sql_old("UPDATE ".SQL."foros_hilos SET text = '".$text."', title = '".$title."'".($_POST['sub_ID'] > 0?", sub_ID = '".$_POST['sub_ID']."'":'').", fecha_programado = ".( $_POST['fecha_programado'] ? "'".$fecha_programado."'" : 'null')." WHERE ID = '".$_POST['subforo']."' AND estado = 'ok' AND (user_ID = '".$pol['user_ID']."' OR 'true' = '".(nucleo_acceso($vp['acceso']['foro_borrar'])?'true':'false')."')");
...
case 'kick':
	...
	if (($_GET[2] == 'quitar') AND ($_GET['ID'])) {
		...
		if ($r['sc'] == true AND nucleo_acceso('supervisores_censo')){
			sql_old("UPDATE kicks SET estado = 'indultado' WHERE pais = '".PAIS."' AND estado = 'activo' AND ID = '".$_GET['ID']."'"); 
			...
			sql_old("UPDATE kicks SET estado = 'cancelado' WHERE pais = '".PAIS."' AND estado = 'activo' AND ID = '".$_GET['ID']."'"); 
			if (mysqli_affected_rows()==1) {
				$result = sql_old("SELECT nick FROM users WHERE ID = '".$kickeado_id."' LIMIT 1");
				while($r = r($result)){ $kickeado_nick = $r['nick'];}
		...
		if ((substr($_POST['nick'], 0, 3) == 'ip-') AND (is_numeric(substr($_POST['nick'], 3)))) {
			...
			$result = sql_old("SELECT nick FROM chats_msg WHERE IP = ".$kick_IP." ORDER BY msg_ID DESC LIMIT 1");
			while($r = r($result)){ $kick_nick = $r['nick']; }
			$_POST['razon'] = '['.$kick_nick.'] '.$_POST['razon'];
			$kick_pais = PAIS;
			$result = sql_old("SELECT ID FROM kicks WHERE pais = '".PAIS."' AND IP = ".$kick_IP." AND estado = 'activo' LIMIT 1");
			while($r = r($result)){ $user_kicked = true; }
			$el_userid = -1;
		} else {
			$result = sql_old("SELECT ID, nick, IP, cargo, pais FROM users WHERE nick = '".$_POST['nick']."' LIMIT 1");
			while($r = r($result)){ $kick_cargo = $r['cargo']; $kick_user_ID = $r['ID']; $kick_nick = $r['nick']; $kick_IP = '\''.$r['IP'].'\''; $kick_pais = $r['pais']; }
			$result = sql_old("SELECT ID FROM kicks WHERE pais = '".PAIS."' AND user_ID = '".$kick_user_ID."' AND estado = 'activo' LIMIT 1");
			while($r = r($result)){ $user_kicked = true; }
...
case 'mensaje-leido':
	if ($_GET['ID'] == 'all') {
		sql_old("UPDATE mensajes SET leido = '1' WHERE recibe_ID = '".$pol['user_ID']."'");
	} elseif ($_GET['ID']) {
		sql_old("UPDATE mensajes SET leido = '1' WHERE ID = '".$_GET['ID']."' AND recibe_ID = '".$pol['user_ID']."'");
	}
	sql_old("UPDATE notificaciones SET visto = 'true' WHERE user_ID = '".$pol['user_ID']."' AND visto = 'false' AND texto LIKE 'Mensaje %'");
	$refer_url = 'msg';
	break;

case 'borrar-mensaje':
	if (is_numeric($_GET['ID'])) {
		sql_old("DELETE FROM mensajes WHERE ID = '".$_GET['ID']."' AND recibe_ID = '".$pol['user_ID']."' LIMIT 1");
		sql_old("UPDATE notificaciones SET visto = 'true' WHERE user_ID = '".$pol['user_ID']."' AND visto = 'false' AND texto LIKE 'Mensaje %'");
		$refer_url = 'msg';
	}
	break;
...
case 'enviar-mensaje':
	...
	if ((!$_GET[2]) AND ($_POST['text']) AND ($_POST['para'])) {
		$text = gen_text($_POST['text'], 'plain');
		if (($_POST['para'] == 'ciudadano') AND ($_POST['nick'])) {
			...
			$result = sql_old("SELECT ID, pais FROM users WHERE nick IN ('".implode("','", $enviar_nicks)."') AND estado != 'expulsado'");
			while($r = r($result)){ 
				sql_old("INSERT INTO mensajes (envia_ID, recibe_ID, time, text, leido, cargo) VALUES ('".$pol['user_ID']."', '".$r['ID']."', '".$date."', '".$text."', '0', '".$_POST['calidad']."')");
				...
				evento_chat('<b>[MP]</b> <a href="/msg">Nuevo mensaje privado</a> <span style="color:grey;">('.$pol['nick'].')</span>', $r['ID'], -1, false, 'p', $r['pais']);
			...
			foreach ($sc AS $user_ID => $nick) {
				if ($user_ID != $pol['user_ID']) {
					sql_old("INSERT INTO mensajes (envia_ID, recibe_ID, time, text, leido, cargo, recibe_masivo) VALUES ('".$pol['user_ID']."', '".$user_ID."', '".$date."', '<b>Mensaje multiple: Supervisor del Censo</b><br />".$text."', '0', '".$_POST['calidad']."', 'SC')");
					evento_chat('<b>[MP]</b> <a href="/msg">Nuevo mensaje privado</a> <span style="color:grey;">(multiple)</span>', $user_ID, -1, false, 'p');
			...
			$result = sql_old("SELECT nombre FROM cargos WHERE pais = '".PAIS."' AND cargo_ID = '".$_POST['cargo_ID']."' LIMIT 1");
			while($r = r($result)){ $cargo_nombre = $r['nombre']; }

			if ($_POST['cargo_ID'] == '55') {
				$result = sql_old("SELECT user_ID FROM cargos_users WHERE pais = '".PAIS."' AND cargo = 'true' AND cargo_ID IN (55, 56, 57) LIMIT 1000");
			} else {
				$result = sql_old("SELECT user_ID FROM cargos_users WHERE pais = '".PAIS."' AND cargo = 'true' AND cargo_ID = '".$_POST['cargo_ID']."' LIMIT 1000");
			}
			while($r = r($result)){ 
				if (($r['user_ID'] != $pol['user_ID']) AND ($r['user_ID'] != 0)) {
					sql_old("INSERT INTO mensajes (envia_ID, recibe_ID, time, text, leido, cargo, recibe_masivo) VALUES ('".$pol['user_ID']."', '".$r['user_ID']."', '".$date."', '<b>Mensaje multiple: ".$cargo_nombre."</b><br />".$text."', '0', '".$_POST['calidad']."', '".$_POST['cargo_ID']."')");
					evento_chat('<b>Nuevo mensaje privado</b> (<a href="/msg"><b>Leer!</b></a>)', $r['user_ID'], -1, false, 'p');
			...
			$result = sql_old("SELECT nombre FROM grupos WHERE grupo_ID = '".$_POST['grupo_ID']."' LIMIT 1");
			while($r = r($result)){ $grupo_nombre = $r['nombre']; }

			$result = sql_old("SELECT ID AS user_ID, grupos FROM users WHERE estado = 'ciudadano' AND pais = '".PAIS."' AND grupos != '' AND grupos LIKE '%".$_POST['grupo_ID']."%' LIMIT 1000");
			while($r = r($result)){ 
				if (($r['user_ID'] != $pol['user_ID']) AND (in_array($_POST['grupo_ID'], explode(' ', $r['grupos'])))) {
					
					sql_old("INSERT INTO mensajes (envia_ID, recibe_ID, time, text, leido, cargo, recibe_masivo) VALUES ('".$pol['user_ID']."', '".$r['user_ID']."', '".$date."', '<b>Mensaje multiple: grupo ".$grupo_nombre."</b><br />".$text."', '0', '".$_POST['calidad']."', '".$_POST['cargo_ID']."')");
			...
			$result = sql_old("SELECT ID FROM users WHERE estado = 'ciudadano' AND pais = '".PAIS."'");
			while($r = r($result)){ 
				sql_old("INSERT INTO mensajes (envia_ID, recibe_ID, time, text, leido, cargo) VALUES ('".$pol['user_ID']."', '".$r['ID']."', '".$date."', '".$text."', '0', '".$_POST['calidad']."')");
...
case 'partido-lista':
	...
	if (($b) AND ($ID_partido) AND ($pol['config']['elecciones_estado'] != 'elecciones')) {
		...
		$result = sql_old("SELECT ID_presidente, siglas FROM partidos WHERE pais = '".PAIS."' AND ID = '".$ID_partido."' AND ID_presidente = '".$pol['user_ID']."' LIMIT 1");
		while($r = r($result)){
			$siglas = $r['siglas'];
			if ($b == 'edit') {
				$text = str_replace('<br />', '', $_POST['html_doc']);
				sql_old("UPDATE partidos SET descripcion = '".gen_text($text)."' WHERE pais = '".PAIS."' AND ID = '".$ID_partido."'");
				...
				sql_old("DELETE FROM partidos_listas WHERE pais = '".PAIS."' AND user_ID = '".$_POST['user_ID']."' AND ID_partido = '".$ID_partido."' LIMIT 1");
			} elseif (($b == 'ceder-presidencia') AND ($_POST['user_ID'])) {
				sql_old("UPDATE partidos SET ID_presidente = '".$_POST['user_ID']."' WHERE pais = '".PAIS."' AND ID = '".$ID_partido."'");
			} elseif (($b == 'del-afiliado') AND ($_POST['user_ID'])) {
				sql_old("UPDATE users SET partido_afiliado = '0' WHERE partido_afiliado = '".$ID_partido."' AND ID = '".$_POST['user_ID']."'");
			}
...
case 'cargo':
	...
	if (($_GET[2] == 'dimitir') AND (is_numeric($_GET['ID'])) AND (nucleo_acceso('cargo', $_GET['ID']))) {

		cargo_del($_GET['ID'], $pol['user_ID'], false);

		$result = sql_old("SELECT nombre FROM cargos WHERE pais = '".PAIS."' AND cargo_ID = '".$_GET['ID']."' LIMIT 1");
		...
		sql_old("UPDATE cargos_users SET cargo = 'false', aprobado = 'no' WHERE pais = '".PAIS."' AND cargo_ID = '".$_GET['ID']."' AND user_ID = '".$pol['user_ID']."'");

		// Asigna al siguiente en la cadena de sucesión si el cargo es electo
		$result = sql_old("SELECT cargo_ID FROM cargos WHERE pais = '".PAIS."' AND cargo_ID = '".$_GET['ID']."' AND elecciones IS NOT NULL LIMIT 1");
		while($r = r($result)){
			// El cargo es electo.
			$result2 = sql_old("SELECT ejecutar FROM votacion WHERE pais = '".PAIS."' AND tipo = 'elecciones' AND estado = 'end' AND cargo_ID = '".$_GET['ID']."' ORDER BY time DESC LIMIT 1");
			while($r2 = r($result2)){
				// Ultimas elecciones legitimas.
				sql_old("UPDATE users SET temp = NULL");
				foreach (explode(':', explodear('|', $r2['ejecutar'], 3)) AS $data) {
					sql_old("UPDATE users SET temp = ".explodear('.', $data, 1)." WHERE pais = '".PAIS."' AND nick = '".explodear('.', $data, 0)."'");
				}
				
				$asignado = false;
				$result3 = sql_old("SELECT ID,
...
FROM users WHERE pais = '".PAIS."' AND estado = 'ciudadano' AND temp IS NOT NULL ORDER BY temp DESC, fecha_registro ASC");
	...
	} elseif (($_GET[2] == 'editar') AND (nucleo_acceso($vp['acceso']['control_cargos']))) {
		$result = sql_old("SELECT * FROM cargos WHERE pais = '".PAIS."' AND asigna > 0");
		while($r = r($result)){
			...
			if ((strlen($_POST['nombre_'.$r['cargo_ID']]) >= 3) AND (is_numeric($_POST['asigna_'.$r['cargo_ID']])) AND (is_numeric($_POST['nivel_'.$r['cargo_ID']])) AND (entre($_POST['nivel_'.$r['cargo_ID']], 1, 99))) {
				...
				sql_old("UPDATE cargos SET nombre = '".$_POST['nombre_'.$r['cargo_ID']]."', nombre_extra = '".strip_tags($_POST['nombre_extra_'.$r['cargo_ID']])."', asigna = '".$_POST['asigna_'.$r['cargo_ID']]."', nivel = '".$_POST['nivel_'.$r['cargo_ID']]."', autocargo = '".($_POST['autocargo_'.$r['cargo_ID']]?'true':'false')."'".$sql_set." WHERE pais = '".PAIS."' AND cargo_ID = '".$r['cargo_ID']."'");
				sql_old("UPDATE examenes SET titulo = '".$_POST['nombre_'.$r['cargo_ID']]."' WHERE pais = '".PAIS."' AND cargo_ID = '".$r['cargo_ID']."'");
			}
		}
		if ($_POST['editar_elecciones'] == 'true') { $refer_url = 'cargos/editar/elecciones'; } else { $refer_url = 'cargos/editar'; }


	} elseif (($_GET[2] == 'eliminar') AND (nucleo_acceso($vp['acceso']['control_cargos'])) AND (is_numeric($_GET['cargo_ID']))) {
...
FROM cargos WHERE pais = '".PAIS."' AND cargo_ID = '".$_GET['cargo_ID']."' AND asigna > 0 LIMIT 1");
		while($r = r($result)){
			if ($r['cargo_num'] == 0) {
				sql_old("DELETE FROM cargos WHERE pais = '".PAIS."' AND cargo_ID = '".$_GET['cargo_ID']."' LIMIT 1");
				sql_old("DELETE FROM cargos_users WHERE pais = '".PAIS."' AND cargo_ID = '".$_GET['cargo_ID']."'");
				sql_old("DELETE FROM examenes WHERE pais = '".PAIS."' AND cargo_ID = '".$_GET['cargo_ID']."' LIMIT 1");
			}
		}
		...
		$result = sql_old("SELECT cargo_ID, asigna, nombre FROM cargos WHERE pais = '".PAIS."' AND cargo_ID = '".$cargo_ID."' LIMIT 1");
		while($r = r($result)){
			if (nucleo_acceso('cargo', $r['asigna'])) { 
				$result2 = sql_old("SELECT nick, online, fecha_registro FROM users WHERE ID = '".$_POST['user_ID']."' AND pais = '".PAIS."' LIMIT 1");
...
case 'eliminar-partido':
	if (($pol['config']['elecciones_estado'] != 'elecciones')) {
		$result = sql_old("SELECT ID, siglas FROM partidos WHERE pais = '".PAIS."' AND ID_presidente = '".$pol['user_ID']."' LIMIT 1");
		while($r = r($result)){
			sql_old("DELETE FROM partidos WHERE pais = '".PAIS."' AND ID = '".$r['ID']."' LIMIT 1");
			sql_old("DELETE FROM partidos_listas WHERE pais = '".PAIS."' AND ID_partido = '".$r['ID']."' LIMIT 1");
...
case 'eliminar-documento':
	...
	while($r = r($result)){ 
		if ((nucleo_acceso($r['acceso_escribir'], $r['acceso_cfg_escribir'])) OR (nucleo_acceso($vp['acceso']['control_docs']))) {
			sql_old("UPDATE docs SET estado = 'del' WHERE ID = '".$r['ID']."'");
...
case 'editar-documento':
	if (($_POST['titulo']) AND ($_POST['cat'])) {
		...
		while($r = r($result)){ 
			...
			if ((nucleo_acceso($r['acceso_escribir'], $r['acceso_cfg_escribir'])) OR (nucleo_acceso($vp['acceso']['control_docs']))) {
				sql_old("UPDATE docs SET cat_ID = '".$_POST['cat']."', text = '".$text."', title = '".$_POST['titulo']."', time_last = '".$date."', acceso_leer = '".$_POST['acceso_leer']."', acceso_escribir = '".$_POST['acceso_escribir']."', acceso_cfg_leer = '".$_POST['acceso_cfg_leer']."', acceso_cfg_escribir = '".$_POST['acceso_cfg_escribir']."', version = version + 1 WHERE ID = '".$r['ID']."'");
			}
...
case 'crear-documento':
	if ((entre(strlen($_POST['title']), 1, 80)) AND (isset($_POST['cat']))) {
		...
		$result = sql_old("SELECT ID FROM docs WHERE pais = '".PAIS."' AND url = '".$url."' LIMIT 1");
		while($r = r($result)) { $url .= '_'.time(); }

		sql_old("INSERT INTO docs 
...
/*		$result = sql_old("SELECT ID FROM docs WHERE pais = '".PAIS."' AND url = '".$url."' LIMIT 1");
		while($r = r($result)){ sql_old("UPDATE docs SET pad_ID = '".$r['ID'].".".rand(100000,999999)."' WHERE ID = '".$r['ID']."'"); }
*/
		actualizar('contador_docs');
...
case 'afiliarse':
	if (($pol['config']['elecciones_estado'] != 'elecciones')) {
		sql_old("UPDATE users SET partido_afiliado = '".$_POST['partido']."' WHERE ID = '".$pol['user_ID']."'");
		sql_old("DELETE FROM partidos_listas WHERE pais = '".PAIS."' AND user_ID = '".$pol['user_ID']."'");
...
case 'crear-partido':
	...
	$result = sql_old("SELECT ID FROM partidos WHERE pais = '".PAIS."' AND ID_presidente = '".$pol['user_ID']."'");
...
");
	...
	while($r = r($result)) {
		sql_old("UPDATE config SET valor = '".$r['num']."' WHERE pais = '".PAIS."' AND dato = 'info_partidos'");
	}
