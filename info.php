...
case 'censo':
	...
	if ((!is_numeric($_GET[3])) AND ($_GET[2] == 'busqueda')) {
		$pagina = $_GET[4];
		$pagina_url = '/info/censo/busqueda/' . $_GET[3] . '/';
	} elseif (($_GET[2]) AND (!is_numeric($_GET[2]))) { 
		$pagina = $_GET[3];
		$pagina_url = '/info/censo/' . $_GET[2] . '/';
	} else { 
		$pagina = $_GET[2]; 
...
<th style="font-size:18px;"><a href="/info/censo/perfiles">'._('Perfiles').'</a></th>
	...
	switch ($_GET[2]) {
		case 'busqueda': $order_by = 'WHERE (text LIKE \'%'.$_GET[3].'%\' OR nombre LIKE \'%'.$_GET[3].'%\' OR nick LIKE \'%'.$_GET[3].'%\' OR datos LIKE \'%'.$_GET[3].'%\') ORDER BY fecha_last DESC'; break;
...
case 'estadisticas':
	echo '<a href="/estadisticas/"><b>'._('Nuevas estadísticas').'...</b></a>';
	break;
