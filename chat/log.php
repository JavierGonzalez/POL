...
while ($r = r($result)) { 
    ...
    $txt_nav = array('/chat'=>_('Chats'), '/chats/'.$r['url']=>$r['titulo'], _('Log'));
    ...
    if ((nucleo_acceso($r['acceso_leer'], $r['acceso_cfg_leer'])) AND (isset($pol['user_ID']))) {
        ...
        $result2 = sql("SELECT * FROM chats_msg WHERE chat_ID = '".$r['chat_ID']."' AND tipo != 'p' ORDER BY msg_ID ASC LIMIT 2000");
        while ($r2 = r($result2)) { 
