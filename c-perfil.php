...
' . $estudios . '
		...
		if ($user_ID != $pol['user_ID']) {
			$txt .= '<p>' . boton('Enviar mensaje', 'http://'.strtolower($pol['pais']).'.virtualpol.com/msg/' . strtolower($nick) . '/') . ' &nbsp; ' . boton('Transferir '.MONEDA_NOMBRE.'', '/pols/transferir/' . strtolower($nick) . '/') . '</p>';
		}
		$txt .= '</div>';
