...
} else if ($_GET[2] == 'confianza-mutua') {
...
} else if ($_GET[2] == 'factores-secundarios') {
...
if ($_GET[2] == 'expulsar') { // /control/expulsiones/expulsar

	$txt_title = 'Expulsar: '.$_GET[3];
	...
	if (is_numeric(str_replace('-', '', $_GET[3]))) {
		$nicks = array();
		$result = sql("SELECT nick FROM users WHERE ID IN ('".implode("','", explode('-', $_GET[3]))."') AND estado != 'expulsado'");
		while ($r = r($result)) { $nicks[] = $r['nick']; }
		$_GET[3] = implode('-', $nicks);
	}
...
<input type="text" value="'.str_replace('-', ' ', $_GET[3]).'" name="nick" size="50" maxlength="900" style="font-weight:bold;" required />
<br /><br /></li>
...
} elseif (($_GET[2] == 'info') AND ($_GET[3]) AND (isset($sc[$pol['user_ID']]))) {
...
FROM kicks
WHERE pais = '".PAIS."' AND ID = '".$_GET[3]."' LIMIT 1");
		while($r = r($result)){
			echo '
...
'.($_GET[3]?'<input type="hidden" name="chat_ID" value="'.$_GET[3].'" />':'').'
<ol>
