...
case 'aceptar-condiciones':
	...
	while($r = mysql_fetch_array($result)) {
		...
		evento_chat('<b>[#] '.crear_link($pol['nick']).' ha aceptado las <a href="http://www'.DEV.'.'.URL.'/legal">Condiciones de Uso de VirtualPol</a></b>.');
	}
	$refer_url = '';
	break;
...
case 'eliminar-documento':
	...
	while($r = mysql_fetch_array($result)){ 
		mysql_query("UPDATE ".SQL."docs SET estado = 'del' WHERE ID = '".$r['ID']."' LIMIT 1", $link);
		evento_log(8, $r['ID']);
		$refer_url = 'doc/';
	}
