...
if ($user_ID == $pol['user_ID']) { //es USER
	...
	$txt .= '<tr><td><p style="text-align:center;"><button onclick="$(\'#editarperfil\').slideToggle(\'slow\');" style="font-weight:bold;" class="pill">'._('Editar perfil').'</button> '.boton(_('Opciones de usuario'), REGISTRAR.'login.php?a=panel').' '.($pol['pais']!='ninguno'?boton(_('Cambiar ciudadanía'), REGISTRAR, false, 'red').' ':'').'<p>
