...
} elseif ($_GET[1] == 'editar') {
	$txt_nav = array('/foro'=>'Foro', 'Editar');
	echo foro_enviar($_GET[2], $_GET[3], true);

} elseif ($_GET[1] == 'mis-respuestas') {


	if ($_GET[2]) {
		$result = mysql_query_old("SELECT ID, nick FROM users WHERE nick = '".$_GET[2]."' LIMIT 1", $link);
		while($r = mysqli_fetch_array($result)) {
			$el_nick = $r['nick'];
			$el_user_ID = $r['ID'];
		}
...
} elseif ($_GET[1] == 'ultima-actividad') {
...
} elseif ($_GET[2]) {			//foro/subforo/hilo-prueba
...
ORDER BY ".($_GET[3]=='mejores'?'votos DESC LIMIT 100':'time ASC LIMIT '.mysqli_real_escape_string($link,$p_limit)), $link);
			while($r2 = mysqli_fetch_array($result2)) {
...
} elseif ($_GET[1] == 'papelera') { //foro/papelera/
