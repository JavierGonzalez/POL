...
$result = sql_old("SELECT ID, nick, pass, pais FROM users WHERE estado = 'validar' AND nick = '".$_GET['nick']."' AND api_pass = '".$_GET['code']."' COLLATE NOCASE LIMIT 1");
while ($r = r($result)) { 
    ...
    if ($r['pais'] == 'ninguno') {
        sql_old("UPDATE users SET estado = 'turista' WHERE ID = '".$r['ID']."'");
        redirect('/registrar/login/login?user='.$r['nick'].'&pass_md5='.$r['pass'].'&url_http=/registrar');
    } else {
        sql_old("UPDATE users SET estado = 'ciudadano' WHERE ID = '".$r['ID']."'");

        
        $result2 = sql_old("SELECT COUNT(*) AS num FROM users WHERE estado = 'ciudadano' AND pais = '".$r['pais']."' COLLATE NOCASE");
